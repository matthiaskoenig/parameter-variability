...
class BayesModel(Sampler):
    ...
    def __init__(self, prior_loc, prior_scale, compartment='[y_gut]',
                 n_post=2000, **kwargs):
    ...
    def model_bayes_setup(self):
        ...
        with pm.Model() as model:
            k = pm.LogNormal('k', mu=self.prior_loc, sigma=self.prior_scale,
                             initval=1)
            ...
            # likelihood
            pm.LogNormal(name=self.compartment, mu=ode_soln, sigma=sigma,
...
if __name__ == '__main__':
    ...
    BayesModel(
        # Sampler Params
        true_loc=args.sampler_mean, true_scale=args.sampler_variance,
        n_sampler=args.n, parameter_name=args.parameter,
        sampler_steps=args.steps, model_path=args.model_path,

        # Model params
        prior_loc=args.prior_mean, prior_scale=args.prior_variance,
        n_post=args.n_post, compartment=args.compartment)
