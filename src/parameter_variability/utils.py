...
def uuid_generator() -> str:
    # use 42 bits (ceil(log2(36^8)) ≈ 42) -> take 42 bits from uuid4
    bits = 42
    n = uuid.uuid4().int & ((1 << bits) - 1)
    # encode to 8-chars base36 (pad with leading zeros if any)
    s = []
    base = len(ALPHABET36)
    for _ in range(8):
        s.append(ALPHABET36[n % base])
        n //= base
