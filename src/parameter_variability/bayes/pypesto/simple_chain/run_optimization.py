...
def optimize_petab_xps(exp_type: str):
    ...
    df.to_csv(RESULTS_SIMPLE_CHAIN / f"xps_{exp_type}.tsv", sep="\t", index=False)
    console.print(df)
    return df
...
def visualize_timepoints_samples():
    ...
    for xp_key in ["n", "Nt"]:
        ...
        # data processing
        df = pd.read_csv(RESULTS_SIMPLE_CHAIN / f"xps_{xp_key}.tsv", sep="\t")
        ...
        ax.legend()
        f.savefig(RESULTS_SIMPLE_CHAIN / f"xps_{xp_key}.png", bbox_inches="tight")
...
def visualize_priors():
    ...
    # data processing
    df = pd.read_csv(RESULTS_SIMPLE_CHAIN / f"xps_prior.tsv", sep="\t")
    df["category"] = df.pid.str.split("_").str[-1]
    ...
    ax.legend()
    plt.show()
    f.savefig(RESULTS_SIMPLE_CHAIN / f"xps_prior.png", bbox_inches="tight")
