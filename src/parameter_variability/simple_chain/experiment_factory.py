...
def simple_chain_experiment_factory(
    ...
    if n_timepoints is None:
        n_timepoints = [20]
        console.print(f"Using default number of timepoints: {n_timepoints}",
                      style="warning")
    ...
    for kt in track(range(len(tuples)),
                    description="Creating experiment definitions..."):
...
if __name__ == "__main__":
    ...
    for key, definition in definitions.items():
        ...
        create_petabs(xps, directory=RESULTS_SIMPLE_CHAIN / f"xps_{key}", show_plot=False)
        console.print()
